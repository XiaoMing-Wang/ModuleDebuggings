...
- (id)performTarget:(NSString *)targetName
  ...
  shouldCacheTarget:(BOOL)shouldCacheTarget {
    ...
    SEL actionS = NSSelectorFromString(actionSuffix);
    
    if (target == nil) {
        [self NoTargetActionResponse:targetClassString selectorString:actionString originParams:params];
        return nil;
    }
    
    /** 缓存target */
    if (shouldCacheTarget) self.cachedTarget[targetClassString] = target;
    
    if ([target respondsToSelector:action] || [target respondsToSelector:actionS] ) {
        SEL realSEL = [target respondsToSelector:action] ? action : actionS;
